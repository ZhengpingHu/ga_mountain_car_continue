#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# MountainCar GA Client: Energy Shaping (No Hidden) + Raw Plotting + Direct Env Input + Continuous Action Fix
...
# ----------------------------
# 3. Neural Network (保持不变)
# ----------------------------
class NNPolicy(nn.Module):
    def __init__(self):
        super().__init__()
        # 无隐藏层结构
        # Input(2) -> [Linear] -> Output(1) -> [Tanh]
        self.net = nn.Sequential(
            nn.Linear(2, 1),   # 唯一的权重层：直接把 2个输入 变成 1个输出
            nn.Tanh()          # 激活函数：必须加在这里，把输出限制在 (-1, 1)
        )
...
# ----------------------------
# 5. Evaluation Function (修改：直接使用环境输入)
# ----------------------------
def evaluate_individual(args):
    ...
    try:
        # 直接创建环境，不需要 visual render
        env = gym.make("MountainCarContinuous-v0")
        obs, _ = env.reset(seed=int(seed))
        
        # obs 就是 [position, velocity]
        ...
        for t in range(max_steps):
            # Direct Input (Already normalized/scaled by environment)
